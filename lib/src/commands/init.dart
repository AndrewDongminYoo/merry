...
/// Whether [relative] is an existing file within [projectPath].
bool _hasFile(String projectPath, String relative) =>
    FileSystemEntity.typeSync(path.join(projectPath, relative)) == FileSystemEntityType.file;

/// Whether [relative] is an existing directory within [projectPath].
...
String _buildTemplate(JsonMap content, String projectPath) {
  bool hasFile(String relative) => _hasFile(projectPath, relative);
  bool hasDir(String relative) => _hasDirectory(projectPath, relative);
  ...
  final isApp = isFlutter && !isPlugin && hasFile('lib/main.dart');
...
dev:
  ...
  (scripts): flutter run
''');
  } else if (isFlutter && hasFile('example/pubspec.yaml')) {
...
dev:
  ...
  (scripts): flutter run
''');
  } else if (!isFlutter && hasFile('bin/${content['name']}.dart')) {
...
''');
  ...
  if (isFlutter && (hasFile('l10n.yaml') || (flutterConfig is Map && flutterConfig['generate'] == true))) {
    buffer.write('''
  ...
  if (isApp) {
    ...
    final entries = [
      for (final entry in targets.entries)
        if (hasDir(entry.key)) ...entry.value,
    ];
